...
combinedXValues = zeros(length(mls), numPlotted);

combinedYValues = zeros(length(mls), numPlotted);

combinedZValues = zeros(length(mls), numPlotted);

combinedColorValues = rand(length(mls), 3).*0.75;

for ml_index = 1:length(mls)  
    ...
    %Generate Color Values based on m_l
        color = combinedColorValues(ml_index, :);
    ...
    % Add current plot to combined plot
        
        combinedXValues(ml_index, :) = xValues;
        combinedYValues(ml_index, :) = yValues;
        combinedZValues(ml_index, :) = zValues;
...
%% ----- Plot Combined Plot -----
    ...
    for ml_index = 1:length(mls)
        s = scatter3(combinedXValues(ml_index, :), combinedYValues(ml_index, :), combinedZValues(ml_index, :), 'filled', 'SizeData',5, 'MarkerFaceAlpha',0.3);
        s.CData = combinedColorValues(ml_index, :);
    end
    pbaspect([1 1 1]);
    campos([2 2 1]);
...
function P_l_ml = generateAssociatedLegendrePolymomial(l, ml)
    ...
    extraPolynomial = sin(theta)^abs(ml);
